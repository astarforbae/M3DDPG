...
def parse_args():
    ...
    parser.add_argument("--bad-policy", type=str,
    ...
    parser.add_argument("--load-good", type=str, default="",
                        help="which good policy to load")
    parser.add_argument("--load-bad", type=str, default="",
...
def train(arglist):
    ...
    with U.single_threaded_session():
        ...
        print('Using good policy {} and bad policy {} with {} adversaries'.format(
            arglist.good_policy, arglist.bad_policy, num_adversaries))
            ...
            if arglist.load_name == "":
                ...
                for i in range(num_adversaries):
                    bad_var_list += tf.get_collection(
                        tf.GraphKeys.GLOBAL_VARIABLES, scope=trainers[i].scope)
